...
if(isset($_GET['code'])){
									...
									include('config.php');
									$invno=$_GET['code'];
									$sql="select billto,email,aemail,name,dt,ship,qty from inv_main,cust_master,inv_child where inv_main.invno='$invno' and billto=cust_master.code and inv_child.invno=inv_main.invno";
									//echo $sql;
						...
						<tr>
							<td style='padding-left:40px;padding-top:20px;padding-bottom:20px'>$email<br>$aemail</td>
									...
									if(!$mail->send()) {
										header("location:invoice-search.php?fail=no");
			
									} else {
										header("location:invoice-search.php?flag=$invno");
									}
									//mail ends here..
