      ...
      <div class="container-fluid">
          ...
          <div class="col-md-12">
            ...
            <div class="card">
              <div class="card-header" style="background-image: linear-gradient( 109.6deg,  rgba(177,248,146,1) 11.2%, rgba(81,152,11,1) 100.1% );">
                <h3 class="card-title" style="black">Forex Invoice Entry</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form class="form-horizontal" method="post" id="formfield" action="forexinvinput.php">
				  <?php
					include('config.php');
					$query=mysqli_query($con,"SELECT MAX(cast(substr(invno,13) as decimal)) as invno1 FROM forexinv_main"); 
		...
		$update= false;
			...
			$code=$_GET['edit'];
					//echo $code;
			$update= true;
			$qry="SELECT * FROM `forexinv_main` WHERE `invno` = '$code'";
...
<label for="hf-password" class="form-control-label">&nbsp;&nbsp;&nbsp;SEZ Invoice</label>
				...
				<!--	<div id="irndiv" <?php if($irnno==''){ echo "style='display:none'";}?> class="col-md-6">
				 ...
				 <div class="row form-group">
                                                <div class="col col-md-3">
                                                    <label for="hf-password" class=" form-control-label">Country</label>
                                                </div>
                                                <div class="col-12 col-md-4">
													<select name="country" id="country" class="select2" style="width:100%">
														<?php if($country<>''){ ?><option><?php echo $country; ?></option><?php } ?>
														<option value="">~~SELECT~~</option>
														<option>India</option>
														<option>Other</option>
													</select>
			...
			$code=$_GET['edit'];
			$sql="select * FROM `forexinv_child` where invno='$code'";
...
//$('#tsgst').val(samt);
$('#tigst').val(iamt);
var gttl=parseFloat(netamt)+parseFloat(iamt);
...
$("#frxinventry").addClass("active");
$("#frxinventry").css("background-color","F0F0F0");
});
