          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header" style="background-image: linear-gradient( 109.6deg,  rgba(177,248,146,1) 11.2%, rgba(81,152,11,1) 100.1% );">
                <h3 class="card-title">Forex Invoice Search</h3>
			...
			if(isset($_POST['search'])){ $frmdt=$_POST['fromdate']; $todt=$_POST['todate'];$cust=$_POST['cust']; $billno=$_POST['billno']; }else{ $frmdt=""; $todt="";$cust="";$billno="";}
					 ...
					 $query = mysqli_query($con,"SELECT code FROM cust_master where name='$cust'");
									...
									$cust = $results['code']; 
					  $sql="SELECT * FROM `forexinv_main` WHERE 1=1";
						if($frmdt<>"" && $todt<>""){
								$sql=" and `dt` between '".$frmdt."' and '".$todt."'";
							 }
						if($cust<>""){	
							$sql=$sql." and `billto`='$cust'";
						}
						if($billno<>""){	
								$sql=$sql." and `invno`='$billno'";
						}
					  	$sql=$sql." order by invno desc";			
			  		  $qry=mysqli_query($con,'select count(*) from `forexinv_main`');
						...
						{ 
							...
							?>
											...
											<tr>
												...
												<td><a href ="forexinv-entry.php?edit=<?php echo $row['invno'];?>" <?php if($invst=='Invoice Cancelled'){echo "class='disabled'";} ?>><button class="btn btn-default btn-sm" type="button" data-toggle="tooltip" data-placement="top" title="Edit" <?php if($invst=='Invoice Cancelled'){echo "disabled";} ?>>
                                                            <i class="far fa-edit"></i>
									</button></a></td>
								<td><a href="forexinvinput.php?delete=<?php echo $row['invno'];?>" <?php if($invst=='Invoice Cancelled'){echo "class='disabled'";} ?>><button class="btn btn-default btn-sm" type="button" data-toggle="tooltip" data-placement="top" title="Delete" onclick="return confirm('Are you sure you want to delete this invoice?')" <?php if($invst=='Invoice Cancelled'){echo "disabled";} ?>>
                                                            <i class="far fa-trash-alt"></i>
									</button></a></td>
										<!--<td><a href="forexprint.php?code=<?php echo $row['invno'];?>" <?php if($invst=='Invoice Cancelled'){echo "class='disabled'";} ?>><button class="btn btn-default btn-sm" type="button" data-toggle="tooltip" data-placement="top" title="Print" <?php if($invst=='Invoice Cancelled'){echo "disabled";} ?>>
                                                            <i class="fas fa-print"></i>
									</button></a></td>-->	
										
											<!--<td><a href="invmail.php?code=<?php echo $row['invno'];?>" <?php if($invst=='Invoice Cancelled'){echo "class='disabled'";} ?>><button class="btn btn-default btn-sm" type="button" data-toggle="tooltip" data-placement="top" title="Mail" <?php if($invst=='Invoice Cancelled'){echo "disabled";} ?>>
...
if(isset($_GET['flag'])){
$invno=$_GET['flag'];
$qr=mysqli_query($con,"select email,aemail from inv_main,cust_master where invno='$invno' and billto=cust_master.code");	
...
$("#frxinvreport").addClass("active");
//$("#invreport").css("background-color","F0F0F0");
});
