					...
					<div class="row form-group">
                                                ...
                                                <div class="col-12 col-md-9">
                    								...
                    								<option value="">~~SELECT~~</option>
													 <?php
                     include('config.php');
                       $res= mysqli_query($con,"select distinct crno from credit_main order by crno asc");
								?>
                       ...
                       {
                      ?>
                      <option><?php echo $row["crno"]; ?></option>
                      <?php
                      }
                      ?>
                    							 </select>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header"style="background-image: linear-gradient( 109.6deg,  rgba(177,248,146,1) 11.2%, rgba(81,152,11,1) 100.1% );">
                <h3 class="card-title">Credit Entry Search</h3>
			...
			if(isset($_POST['search'])){ $frmdt=$_POST['fromdate']; $todt=$_POST['todate'];$cust=$_POST['cust']; $billno=$_POST['billno']; }else{ $frmdt=""; $todt="";$cust="";$billno="";}
				...
				if(($frmdt<>"")&&($todt<>"")&&($cust<>"")&&($billno<>"")){
						$sql="SELECT * FROM `credit_main` WHERE `crdt` between '".$frmdt."' and '".$todt."' and `billto`='$cust' and `crno`='$billno' order by crno desc";
					 }
					elseif(($frmdt<>"")&&($todt<>"")&&($cust=="")&&($billno=="")){
						$sql="SELECT * FROM `credit_main` WHERE `crdt` between '".$frmdt."' and '".$todt."' order by crno desc";
					 }  
				elseif(($frmdt<>"")&&($todt<>"")&&($cust<>"")&&($billno=="")){	
								
						$sql="SELECT * FROM `credit_main` WHERE `crdt` between '".$frmdt."' and '".$todt."' and `billto`='$cust' order by crno desc";
				}
					  
	 			elseif(($frmdt<>"")&&($todt<>"")&&($cust=="")&&($billno<>"")){	
								
						$sql="SELECT * FROM `credit_main` WHERE `crdt` between '".$frmdt."' and '".$todt."' and `crno`='$billno' order by crno desc";
				}
					  
				elseif(($frmdt=="")&&($todt=="")&&($cust=="")&&($billno<>"")){	
								
						$sql="SELECT * FROM `credit_main` WHERE  `crno`='$billno' order by crno desc";
				}
					  				
			  elseif(($frmdt=="")&&($todt=="")&&($cust<>"")&&($billno=="")){	
								
					$sql="SELECT * FROM `credit_main` WHERE  `billto`='$cust' order by crno desc";
			  }
					  				
							
							else{
							$sql="SELECT *  FROM `credit_main` order by crno desc ";
							
							}
					 // echo $sql;
					  $qry=mysqli_query($con,'select count(*) from `credit_main`');
						...
						{ 
							...
							?>
											...
											<tr style="color:black">
												...
												<td style="color:black">
													<?php 
														$crdt=date("d-m-Y",strtotime($row['crdt']));
														echo $crdt;
													?>
												</td>
												...
												<td style="color:black">
													<?php 
														$invdt=date("d-m-Y",strtotime($row['invdt']));
														echo $invdt;
													?>
												</td>
												<td style="color:black">
												...
												<td style="color:black"><a href ="credit-entry.php?edit=<?php echo $row['crno'];?>"><button class="btn btn-default btn-sm" type="button" data-toggle="tooltip" data-placement="top" title="Edit">
                                                            <i class="far fa-edit"></i>
									</button></a></td>
								<td style="color:black"><a href="credinput.php?delete=<?php echo $row['crno'];?>"><button class="btn btn-default btn-sm" type="button" data-toggle="tooltip" data-placement="top" title="Delete" onclick="return confirm('Are you sure you want to delete this entry?')">
...
if(isset($_GET['flag'])){
$crno=$_GET['flag'];
$qr=mysqli_query($con,"select email,aemail from credit_main,cust_master where crno='$crno' and billto=cust_master.code");	
...
<script>
	$(document).ready(function() {
            		 swal('<?php echo $popupmsg; ?>').then(function() {
 				  			window.location = "http://mksoftservice.com/Naisha/credit-search.php";
							});
				});
